...
INDEX_CAPTION = is_enabled(environ.get('SAVE_CAPTION', "False"), False) # Save caption db when idexing make it False if you dont use USE_CAPTION_FILTER for search results (default: True)
...
reqst_channel = environ.get('REQST_CHANNEL_ID', '-10026621438545')  # Request channel id (make sure bot is admin)
...
# User Configuration
# ============================
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '6725874739 1018033649').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '6725874739 1018033649').split()]
